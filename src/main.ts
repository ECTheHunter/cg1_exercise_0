...
function callback(changed: utils.KeyValuePair<Settings>) {
  if (changed.key == "name") {
    ...
    mesh.material = new THREE.MeshPhongMaterial({color: 0xfff000});
    else
    mesh.material = new THREE.MeshPhongMaterial({color: 0x1f0ff0});
  }
}
...
function randomcolor(){
  var color=Math.floor(Math.random() *  0xFFFFFFFF);
  return  color;
}
function main(){
